    ...
    def _get_engine(self, name: str):
        ...
        if name not in self._engines:
            if name == "tesseract":
                ...
                from ..ocr.qwen_engine import QwenEngine
                self._engines[name] = QwenEngine()
            elif name == "ensemble":
        ...
        if path.exists():
            try:
                ...
                try:
                    ...
                    # If UTF-8 fails, try with 'windows-1252' encoding
                    file_content = path.read_text(encoding="windows-1252")
                
                data = json.loads(file_content)
                return data.get("ocr_text")
            except (json.JSONDecodeError, KeyError) as e:
                print(f"Error loading OCR cache from {path}: {e}")
                return None
        return None
